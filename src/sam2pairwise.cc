...
int main(int argc, char **argv) {
  ...
  string known_mut;
  int opt;
  bool filter_mapped = false;
  while ((opt = getopt(argc, argv, ":fm:l:")) != -1) {
    switch (opt) {
    case 'f':
      filter_mapped = true;
      break;
    ...
    while (subpos < line[9].size()) {
      ...
      if (cigarpos < cigar_number) {
        ...
        try {
          ...
          translate_cigar(modseq, read, cigar_letter, subpos, cigarpos,
                          nonmatch_flag, insert_flag, n_flag, pad_flag);
      ...
      if (!md.empty()) {
        ...
        try {
          ...
          translate_md(mdstream, read, reference, subpos, mdpos, md_number,
                       md_letter, mdintnext_flag, insert_flag, nonmatch_flag,
                       n_flag, pad_flag);
      ...
      if (nonmatch_flag) {
        if (known_mut.size() > 0 &&
            reference.substr(reference.size() - 1, 1) ==
                known_mut.substr(0, 1) &&
            read.substr(subpos, 1) == known_mut.substr(2, 1)) {
          matches += known_mark;
        } else {
          matches += ' ';
        }
